...
#                                       map_location=torch.device('cpu')
#                                       ))
optimizer_ximl = torch.optim.Adam(model_ximl.parameters(), lr=1e-1, weight_decay=1e-5)
epochs = 100
...
test_explanations(model, multilabel_mnist_test_loader, device, 'cosine_similarity')
test_explanations(model_ximl, multilabel_mnist_test_loader, device, 'cosine_similarity')
test_explanations(model, multilabel_mnist_test_loader, device, 'number of high activated pixels')
test_explanations(model_ximl, multilabel_mnist_test_loader, device, 'number of high activated pixels')
